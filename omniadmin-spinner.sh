...
omniadmin_source='https://github.com/exela/secret-sauce/releases/download/v0.1.0-barebones/omni.admin.autologin-barebones-7.4.1.jar'
...
init () {
    ...
    if [[ ! -f "$LRDIR/$omniadmin_jar_name" ]]; then
        ...
        if [[ -f "latest_release.json" ]]; then
            if [ -z "$latest_release" ]; then
                ...
                if [[ -f "$LRDIR/$omniadmin_jar_name" ]]; then
                    ...
                    wget -np -nd -nH -q --show-progress $omniadmin_source -P $LRDIR
                fi
            fi
        else
        ...
        echo -e "Omniadmin jar already exists: $LRDIR/$omniadmin_jar_name"
    fi
...
omniInstall () {
    echo -e "\n---\nApplying OmniAdmin to spinner env: $(basename "$PWD")"
    # Place the Omniadmin Jar
    cp $omniadmin_jar_location/$omniadmin_jar_name $destination
